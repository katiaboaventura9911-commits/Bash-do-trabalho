...
if lista_arquivos:
    ...
    try:
        ...
        if df is not None:
            ...
            # ---------------------------------------------------------
            # EXIBIÇÃO DAS MACROS E CÓDIGO VBA
            # ---------------------------------------------------------
            if extensao in [".xlsm", ".xls", ".xlsb"]:
                st.markdown("---")
                st.header("📜 Visualização de Código VBA / Macros")
                try:
                    vba_parser = VBA_Parser(str(caminho_arquivo_real))
                    if vba_parser.detect_vba_macros():
                        macros_encontradas = False
                        for filename, stream_path, vba_filename, vba_code in vba_parser.extract_macros():
                            if vba_code.strip():
                                macros_encontradas = True
                                with st.expander(f"📌 Módulo / Objeto: `{vba_filename}`"):
                                    st.code(vba_code, language="vbnet")
                        
                        if not macros_encontradas:
                            st.info("ℹ️ O projeto VBA foi localizado, mas não há código escrito nos módulos.")
                    else:
                        st.info("ℹ️ Este arquivo Excel não possui macros VBA.")
                    
                    vba_parser.close()
                except Exception as err_vba:
                    st.error(f"❌ Não foi possível ler as macros VBA: {err_vba}")
